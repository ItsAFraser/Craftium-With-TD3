...
class GumbelMapperActor(Actor):
    ...
    def forward(self, obs: th.Tensor) -> th.Tensor:
        ...
        action_scores = gumbel_softmax(self.mapper(latent)) # update to use gumbel
        mouse = th.tanh(self.mouse_head(features))

        return th.cat([action_scores, mouse], dim=1)


class GumbelMapperTD3Policy(TD3Policy):
    """TD3 policy that uses GumbelMapperActor as its actor network."""

    def make_actor(self, features_extractor=None) -> Actor:
        actor_kwargs = self._update_features_extractor(self.actor_kwargs, features_extractor)
        return GumbelMapperActor(**actor_kwargs).to(self.device)
